...
Horizon will routinely modify your PR, whether it's a quick rebase or to take care
...
Unlike the Horizon API and its implementation, modifications to Paper and Minecraft source files
...
- `purpur-patches` (applies to the `purpur-server/` git repo)
    - `sources`: Per-file patches to Paper API classes;
    - `features`: Larger feature patches that modify multiple Purpur API classes.

Under `horizon-server`:

- `minecraft-patches` (applies to the `horizon-server//` git repo)
...
3. cd into `horizon-server` for server changes, `horizon-api` for API changes,
   `paper-api` for Paper API changes, and `paper-server` for Paper Server changes, 
   `purpur-api` for Purpur API changes, and `purpur-server` for Purpur Server changes

`horizon-server/src/minecraft/java` and `horizon-server/src/minecraft/java/resources` are not git repositories in the traditional sense.
...
by the individual feature-patch commits. `paper-api/`, `paper-server/`, `purpur-api/` and `purpur-server`
...
- `./gradlew applyAllPatches` - Applies all patches defined in the paperweight-patcher project and the server project. (equivalent to running `applyPurpurPatches` and then `applyAllServerPatches` in a second Gradle invocation)
...
- `./gradlew rebuildPurpurSingleFilePatches` - Fixups and rebuilds all paper single-file patches. This is how you'd make changes to the `build.gradle.kts` files located under `horizon-api` and `horizon-server`
...
1. Replace `pick` with `edit` for the commit/patch you want to modify (in this
   case the very first commit, `horizon File Patches`), and
   "save" the changes;
...
optimizations we have. This makes it easier to update Horizon during Minecraft updates,
...
Remember that these changes will affect everyone who runs Horizon, not just you
and your server.
...
All modifications to Minecraft files, Paper and Purpur files should be marked. For historical reasons,
...
```java
entity.getWorld().dontBeStupid(); // Horizon - Move away from beStupid()
...
### Imports
When adding new imports to a Minecraft or Paper or Purpur class, use the fully qualified class name
...
public class SomeVanillaClass {
    public final org.bukkit.Location newLocation; // Horizon - add location
}
```
...
but the visibility is too low (e.g. a private field in an entity class). Horizon can use access transformers
to change the visibility or remove the final modifier from fields, methods, and classes. Inside the `build-data/horizon.at`
...
When submitting feature patches to Horizon, we may ask you to add notes to the patch
...
to make future updates easier on ourselves, Horizon tries to use obfuscation
...
```java
double d0 = entity.getX(); final double fromX = d0; // Horizon - OBFHELPER
// ...   
this.someMethod(fromX); // Horizon
```
